# Cours commande Linux

Créer un fichier vide :

Afficher le contenu d'un fichier :

Renommer ou déplacer un fichier :

Copier un fichier :

Supprimer un fichier :

Ouvrir un fichier avec l'éditeur nano :

Changer le propriétaire d'un fichier :

Changer le groupe d'un fichier :

Changer le propriétaire ET le groupe en même temps :

Changer les permissions d'un fichier :

Rendre un script exécutable :

Créer un lien symbolique :

## Dossiers

Afficher le dossier courant :

Changer de dossier :

Revenir au dossier parent :

Revenir directement dans son dossier personnel :

Créer un dossier :

Créer un dossier et ses sous-dossiers en une seule commande :

Lister le contenu d'un dossier avec les détails et fichiers cachés :

Copier un dossier et tout son contenu :

Supprimer un dossier vide :

Supprimer un dossier non vide et tout son contenu :
